import { createServer, Tool, Prompt, StdioServerTransport } from "@modelcontextprotocol/sdk/server/index.js";
import { FirebaseApp, cert, getApps, initializeApp } from "firebase-admin/app";
...
function ensureFirebase(): InitializedServices {
  if (getApps().length > 0) {
...
const firestoreQueryTool = new Tool({
  ...
  async execute({ collection, where = [], limit, orderBy, order }) {
    ...
    for (const filter of where as any[]) {
